...
**功能描述**：在等待 LLM 响应和工具执行时，显示实时计时器，让用户了解等待时长。

**实现方式**：
- 发送消息后立即启动等待计时器
- 工具执行时显示工具执行计时器
- 工具执行完成后显示"正在处理工具结果"并启动等待计时器
- 每秒更新显示（如：等待响应: 3 秒）
- 收到响应事件后停止计时器
- 使用后台线程更新计时，不阻塞主流程
...
**性能优化**：
- 工具结果超过 10000 字符时自动截断，避免上下文过大导致延迟
- 截断时保留前 10000 字符，并显示原始长度信息

**代码位置**：
- `by19code/cli/renderer.py` - `start_waiting_timer()` / `stop_waiting_timer()` / `start_tool_timer()` / `stop_tool_timer()` 方法
- `by19code/cli/app.py` - `_handle_chat()` 中调用计时器
- `by19code/core/engine.py` - 发送工具执行事件和处理事件，截断过大结果
...
1. **可配置的自动切换模式**
   - 默认禁用（保持等待）
   - 通过 `/switch` 命令切换模式
   - 配置项：`safety.auto_switch_on_timeout`

2. **超时检测机制**
   - 超时阈值：`safety.change_model_time`（默认 60 秒）
   - 检测模型无响应时间
   - 持续 5 秒确认超时后触发切换
...
3. **智能模型选择**
   - 记录每个模型的历史超时次数
   - 优先选择超时次数最少的模型
   - 超时日志持久化到 `~/.by19code/timeout_log.json`
...
[警告] 模型 claude 响应超时，自动切换到 deepseek
...
**超时日志管理**：
- 存储位置：`%USERPROFILE%\.by19code\timeout_log.json`
- 记录格式：`{"claude": 3, "deepseek": 1, "kimi": 0}`
- 自动持久化，跨会话保留
...
**代码位置**：
- `by19code/core/timeout_tracker.py` - 超时跟踪器
- `by19code/core/engine.py` - 超时检测和自动切换逻辑
- `by19code/cli/app.py` - `/switch` 命令处理
- `by19code/config/settings.py` - `SafetyConfig.auto_switch_on_timeout`
